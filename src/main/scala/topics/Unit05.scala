...
object Unit05 {
  ...
  @main def exercise51(): Unit =
    ...
    lazy val f = bitList.foldRight(0)((_, x) => x + 1)
    lazy val g = bitList.foldRight(0)((x, y) => if(x == 1) 1 - y else y)
    lazy val h = bitList.foldRight((0, 0)) { case (z, (x, y)) => (x + 1 - z, y + z) }
    lazy val i = wordList.foldRight("")((x, y) => if(x.length > y.length) x else y)
    lazy val j = wordList.foldRight(List[String]())((x, xs) => x.reverse :: xs)
    lazy val k = wordList.foldRight(List[String]())((x, xs) => if(x.contains('o')) x :: xs else xs)
  ...
  val veryLongVector: Vector[Long] = (1L to 100000L).toVector
