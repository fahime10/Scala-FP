...
object Unit08 {
  ...
  */
  ...
  lazy val p81c: Parser[String] = for _ <- char('<'); s <- alpha.some; _ <- char('>') yield s.mkString
  lazy val p81d: Parser[String] =
    for _ <- char('<'); tag <- alpha.some; _ <- char('>')
      body <- sat(_ != '<').many
      _ <- string("</")
  ...
  */
  @main def exercise83(): Unit =
    ...
    println(s"$f2.evaluate($env2) = ${f1.evaluate(env2)}")
